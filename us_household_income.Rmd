...
# Distribution
ggplot(data = df_income2)+
  geom_histogram(mapping = aes(x = df_income2$Median, fill = df_income2$Type), bins = 40) +
...
```
Our confidence interval for population's median income is [53145.03, 53812.59]. We are 95% confident the true population's median income falls bewteen our confidence interval 95% of the time. Given that our data came from a random sample and assuming the population's distribution is approximately normal.
...
we took a random subsample of fixed size (n=100) from our full data set, we then proceeded to calculate the confidence intervals and hypothesis tests against the true sample mean, record the coverage and type I errors obtained.
...
for (j in 1:n.trials){
  ...
  x.bar <- mean(x.samp)
  x.sd <- sd(x.samp)
  inside[j] <- (mu > x.bar - qt(0.95,n-1)*x.sd/sqrt(n) & mu < x.bar + qt(0.95,n-1)*x.sd/sqrt(n))
}

coverage <- mean(inside)
print(coverage)
```
Our coverage for the true sample mean over 10,000 trials was 88.98%, a little lower than expected (95%), this could be attributed to outliers in the data. 
